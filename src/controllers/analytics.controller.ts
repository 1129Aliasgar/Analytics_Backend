...
export class AnalyticsController extends BaseController{
  ...
  async getUsers(req: Request, res: Response) {
    try {
      ...
      this.error(res, error instanceof Error ? error.message : String(error), STATUS_CODE.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getUserById(req: Request, res: Response) {
    try {
      ...
      this.logger.info(`Fetching user with id ${id} ...`, { req: req.user!.userId });
      const user = await this.analyticsService.getUserById(id);
      this.success(res, { message: "User fetched successfully", user }, STATUS_CODE.OK);
    } catch (error) {
      this.error(res, error instanceof Error ? error.message : String(error), STATUS_CODE.INTERNAL_SERVER_ERROR);
    }
  }
